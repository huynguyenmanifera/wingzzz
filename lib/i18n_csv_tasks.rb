...
module I18nCsvTasks
  ...
  def csv_export(opts = {})
    ...
    router =
      I18n::Tasks::Data::Router::PatternRouter.new(
        nil,
    ...
    translations_by_path.each do |(path, translations_by_locale)|
      ...
      CSV.open(path, 'wb') do |csv|
        csv << (%w[key] + i18n.locales)

        translations_by_locale[i18n.base_locale].keys.each do |key|
          values =
            i18n.locales.map { |locale| translations_by_locale[locale][key] }
  ...
  def csv_import(opts = {})
    i18n.config['csv']['import'].each do |file|
      ...
      CSV.parse(csv, headers: true) do |row|
        ...
        i18n.locales.each do |locale|
          unless row.has_key?(locale)
            raise "Locale missing for key #{key}! (locales in app: #{
                    locales
                  } / locales in file: #{row.headers.inspect})"
          end
      ...
      i18n.data.merge! I18n::Tasks::Data::Tree::Siblings.from_flat_pairs(
                         translations
                       )
    end
  end
